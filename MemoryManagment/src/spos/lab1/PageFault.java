package spos.lab1;/* It is in this file, specifically the replacePage function that will
  ...
  public static void replacePage ( Vector mem , int virtPageNum , int replacePageNum , ControlPanel controlPanel ) 
  {
    ...
    while (count < virtPageNum ) {
      ...
      if ( page.physical != -1 ) {
        if (firstPage == -1) {
          firstPage = count;
          lowestCounter = page.lastUsageCounter;
        }
        if (page.lastUsageCounter < lowestCounter) {
          lowestCounter = page.lastUsageCounter;
          lowestCounterPage = count;
        }
      }
      count++;
    }
    if (lowestCounterPage == -1) {
      lowestCounterPage = firstPage;
    }

    Page page = ( Page ) mem.elementAt( lowestCounterPage );
    Page nextpage = ( Page ) mem.elementAt( replacePageNum );
    controlPanel.removePhysicalPage( lowestCounterPage );
